...
class MedicalImageTextPreprocessor:
    def __init__(self, data_dir):
        ...
        self.images_dir = os.path.join(data_dir, "ChestX-rays", "images", "images_normalized")
        self.reports_file = os.path.join(data_dir, "ChestX-rays", "indiana_reports.csv")
        self.output_dir = os.path.join(os.path.dirname(data_dir), "processed_vqa_data")
    ...
    def process_images(self):
        ...
        # 只保留有有效图像的报告
        self.reports_df = self.reports_df.iloc[valid_indices].reset_index(drop=True)
...
if __name__ == "__main__":
    # 设置数据目录
    data_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "VQA_data")
